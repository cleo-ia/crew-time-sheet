...
export const useAutoSaveTransport = () => {
  ...
  return useMutation({
    mutationFn: async (params: SaveTransportParams) => {
      ...
      // 0. Si pas de ficheId, chercher une fiche existante AVEC salarié (pas de création orpheline)
      ...
      if (!ficheId) {
        ...
        if (existingFiche) {
          ...
          console.log("[useAutoSaveTransport] Aucune fiche salarié trouvée, skip");
          return null;
        }
      }
    ...
    onSuccess: () => {
      ...
      // PAS de toast ici (sauvegarde silencieuse)
    },
    onError: (error) => {
      // Logger uniquement en console
      console.error("Auto-save transport failed:", error);
    },
  });
};
