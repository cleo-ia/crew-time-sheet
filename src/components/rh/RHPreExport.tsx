...
export const RHPreExport = ({ filters }: RHPreExportProps) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Table avec colonnes sticky et barre de scroll horizontale sticky */}
      <div className="border rounded-lg h-[600px] flex flex-col">
        {/* Zone principale avec scroll vertical synchronisé */}
        <div className="flex-1 overflow-y-auto flex">
          {/* Zone FIXE à gauche - Matricule/Nom/Prénom */}
          <div className="flex-shrink-0 z-20 bg-background border-r shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">
                ...
                {rows.map((row, index) => {
                  ...
                  return (
                    <TableRow key={index} className={row.isModified ? "bg-blue-50/30" : ""}>
                      <TableCell className="font-mono text-xs">{data.matricule}</TableCell>
        ...
        {/* Scrollbar horizontale sticky en bas - contrôle la zone scrollable */}
        <div className="flex border-t flex-shrink-0">
          {/* Spacer pour la zone fixe */}
          <div className="w-[340px] flex-shrink-0 bg-muted/30" />
          {/* Scrollbar */}
          <div 
            ref={stickyScrollRef}
