...
import { useVentilationOuvrier, VentilationEmployeeRow } from "@/hooks/useVentilationAnalytique";
import { exportVentilationOuvrierExcel } from "@/lib/ventilationExport";
import { toast } from "sonner";

interface VentilationOuvrierProps {
  filters: {
    periode: string;
  };
}

export const VentilationOuvrier = ({ filters }: VentilationOuvrierProps) => {
  const [search, setSearch] = useState("");
  const { data, isLoading, error } = useVentilationOuvrier(filters.periode);

  // Group data by employee
  const groupedData = useMemo(() => {
    if (!data) return [];
    
    const groups: { salarieId: string; nom: string; prenom: string; typeMO: string; rows: VentilationEmployeeRow[] }[] = [];
  ...
  const filteredGroups = groupedData.filter(group =>
    `${group.nom} ${group.prenom}`.toLowerCase().includes(search.toLowerCase())
  );
  ...
  const handleExport = async () => {
    ...
    try {
      const fileName = await exportVentilationOuvrierExcel(data, filters.periode);
  ...
  if (!filters.periode || filters.periode === "all") {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
        ...
        <p>Sélectionnez un mois pour afficher la ventilation par ouvrier</p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/50">
                ...
                <TableHead className="font-semibold">Type MO</TableHead>
              ...
              {filteredGroups.map((group, groupIdx) => (
                group.rows.map((row, rowIdx) => (
                  ...
                  >
                    ...
                    <TableCell>
                      {!row.isTotal && (
                        <Badge variant={row.typeMO === 'MOAPP' ? 'secondary' : 'outline'}>
                          {row.typeMO}
                        </Badge>
                      )}
                    </TableCell>
      ...
      {filteredGroups.length > 0 && (
        <div className="text-sm text-muted-foreground">
          {employeeCount} salarié(s) • Ouvriers et apprentis uniquement
        </div>
      )}
    </div>
  );
