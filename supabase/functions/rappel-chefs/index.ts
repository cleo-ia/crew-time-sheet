...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!force && !isTargetParisHour(12)) {
      console.log('[rappel-chefs] Pas encore 12h à Paris, skip')
      return new Response(
    ...
    if (rolesError) {
      console.error('[rappel-chefs] Erreur lors de la récupération des roles:', rolesError)
      throw rolesError
    }

    if (!rolesData || rolesData.length === 0) {
      console.log('[rappel-chefs] Aucun chef trouvé')
      return new Response(
    ...
    console.log(`[rappel-chefs] ${chantiersMap.size} chantier(s) récupéré(s)`)

    // ÉTAPE 5: Grouper les fiches par chef ET par chantier
    const fichesByChef = new Map<string, Map<string, number>>()
    for (const fiche of fichesData) {
      if (!fichesByChef.has(fiche.user_id)) {
        fichesByChef.set(fiche.user_id, new Map())
      }
      const chefChantiers = fichesByChef.get(fiche.user_id)!
      const currentCount = chefChantiers.get(fiche.chantier_id) || 0
      chefChantiers.set(fiche.chantier_id, currentCount + 1)
    }

    // ÉTAPE 6: Construire la liste des chefs à notifier
    const chefsANotifier: ChefWithFiches[] = []

    for (const [chefId, chantiersFiches] of fichesByChef) {
      const profile = profilesMap.get(chefId)
      if (!profile) {
        console.warn(`[rappel-chefs] Profile non trouvé pour chef ${chefId}`)
        continue
      }

      // Construire les détails par chantier
      const chantierDetails: Array<{ nom: string; nbFiches: number }> = []
      let totalFiches = 0

      for (const [chantierId, nbFiches] of chantiersFiches) {
        const chantierNom = chantiersMap.get(chantierId) || 'Chantier inconnu'
        chantierDetails.push({ nom: chantierNom, nbFiches })
        totalFiches += nbFiches
      }
      ...
      chefsANotifier.push({
        ...
        nb_fiches_en_cours: totalFiches,
        chantierDetails
      })

      console.log(`[rappel-chefs] Chef ${profile.first_name} ${profile.last_name} : ${totalFiches} fiche(s) en cours sur ${chantierDetails.length} chantier(s)`)
    }
    ...
    for (const chef of chefsANotifier) {
      try {
        ...
        const emailContent = `
          ${createAlertBox(
            `Vous avez <strong>${chef.nb_fiches_en_cours} fiche(s)</strong> non finalisée(s) pour la semaine <strong>${currentWeek}</strong>.`,
            'warning'
          )}
          ...
          ${createClosingMessage('Merci de finaliser vos fiches dès que possible afin de permettre la validation par votre conducteur.')}
        `
    ...
    await supabase.from('rappels_historique').insert({
      ...
      details: {
        ...
        chefs: chefsANotifier.map(c => ({
          ...
          chantiers: c.chantierDetails.map(ch => ({ nom: ch.nom, nb_fiches: ch.nbFiches }))
        })),
        results
      }
    })
